...
require 'fog/identity/telefonica/v3/models/project'

module Fog
  module Identity
    class TeleFonica
      class V3
        class Projects < Fog::TeleFonica::Collection
          model Fog::Identity::TeleFonica::V3::Project

          def all(options = {})
            if service.telefonica_cache_ttl > 0
              cached_project, expires = Fog::Identity::TeleFonica::V3::Project.cache[{:token   => service.auth_token,
                                                                                     :options => options}]
          ...
          def find_by_id(id, options = []) # options can include :subtree_as_ids, :subtree_as_list, :parents_as_ids, :parents_as_list
            ...
            if service.telefonica_cache_ttl > 0
              ...
              cache[{:token => service.auth_token, :id => id, :options => options}] = [
                top_project, Time.now + service.telefonica_cache_ttl
              ]
              Fog::Identity::TeleFonica::V3::Project.cache = cache
            end
            top_project
          ...
          def project_from_hash(project_hash, service)
            Fog::Identity::TeleFonica::V3::Project.new(project_hash.merge(:service => service))
          end
        end
      end
    end
