    ...
    class TeleFonica
      class Real
        ...
        def create_temp_url(container, object, expires, method, options = {})
          ...
          raise ArgumentError, "Storage must be instantiated with the :telefonica_temp_url_key option" if @telefonica_temp_url_key.nil?

          scheme = options[:scheme] || @telefonica_management_uri.scheme
          host = options[:host] || @telefonica_management_uri.host
          port = options[:port] || @telefonica_management_uri.port
          ...
          object_path_escaped   = "#{@path}/#{Fog::TeleFonica.escape(container)}/#{Fog::TeleFonica.escape(object, "/")}"
          object_path_unescaped = "#{@path}/#{Fog::TeleFonica.escape(container)}/#{object}"
          string_to_sign = "#{method}\n#{expires}\n#{object_path_unescaped}"

          hmac = Fog::HMAC.new('sha1', @telefonica_temp_url_key.to_s)
