      ...
      module Real
        def list_snapshots(options = true, options_deprecated = {})
          if options.kind_of?(Hash)
            ...
            if options
              Fog::Logger.deprecation('Calling TeleFonica[:volume].list_snapshots(true) is deprecated, use .list_snapshots_detailed instead')
            else
              Fog::Logger.deprecation('Calling TeleFonica[:volume].list_snapshots(false) is deprecated, use .list_snapshots({}) instead')
            end
