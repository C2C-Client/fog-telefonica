...
module Fog
  module Compute
    class TeleFonica
      class SecurityGroup < Fog::TeleFonica::Model
        identity  :id
        ...
        def security_group_rules
          Fog::Compute::TeleFonica::SecurityGroupRules.new(:service => service).load(attributes[:security_group_rules])
        end
        ...
        def rules=(new_rules)
          Fog::Logger.deprecation('#rules= is deprecated. Use the Fog::Compute::Telefonica::SecurityGroupRules collection to create new rules.')
        ...
        def create_security_group_rule(min, max, ip_protocol = "tcp", cidr = "0.0.0.0/0", group_id = nil)
          Fog::Logger.deprecation('#create_security_group_rule is deprecated. Use the Fog::Compute::Telefonica::SecurityGroupRules collection to create new rules.')
          requires :id
          service.create_security_group_rule(id, ip_protocol, min, max, cidr, group_id)
        end

        def delete_security_group_rule(rule_id)
          Fog::Logger.deprecation('#create_security_group_rule is deprecated. Use the Fog::Compute::Telefonica::SecurityGroupRule objects to destroy rules.')
