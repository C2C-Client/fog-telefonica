      ...
      class Real
        def list_snapshots(options = true)
          if options.kind_of?(Hash)
            ...
            if options
              Fog::Logger.deprecation('Calling TeleFonica[:compute].list_snapshots(true) is deprecated, use .list_snapshots_detail instead')
            else
              Fog::Logger.deprecation('Calling TeleFonica[:compute].list_snapshots(false) is deprecated, use .list_snapshots({}) instead')
            end
