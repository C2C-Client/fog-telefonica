    ...
    class TeleFonica
          ...
          if options['security_groups']
            ...
            data['server']['security_groups'] =
              Array(options['security_groups']).map do |sg|
                name = if sg.kind_of?(Fog::Compute::TeleFonica::SecurityGroup)
                         sg.name
                       else
                         sg
                       end
      ...
      class Mock
        def create_server(name, image_ref, flavor_ref, options = {})
          ...
          identity = Fog::Identity::TeleFonica.new(:telefonica_auth_url => credentials[:telefonica_auth_url], :telefonica_identity_api_version => 'v2.0')
          ...
          user_id = if user
                      ...
                      response = identity.create_user(@telefonica_username,
                                                      'password',
                                                      "#{@telefonica_username}@example.com")
          ...
          if security_groups
            groups = Array(security_groups).map do |sg|
              if sg.kind_of?(Fog::Compute::TeleFonica::SecurityGroup)
                sg.name
              else
                sg
              end
