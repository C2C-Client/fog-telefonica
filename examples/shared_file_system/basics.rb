...
#
# You will need to source TeleFonica credentials since the script
...
auth_options = {
  :telefonica_auth_url     => "#{ENV['OS_AUTH_URL']}/auth/tokens",
  :telefonica_api_key      => ENV['OS_PASSWORD'],
  :telefonica_username     => ENV['OS_USERNAME'],
  :telefonica_domain_name  => ENV['OS_USER_DOMAIN_NAME'],
  :telefonica_project_name => ENV['OS_PROJECT_NAME'],
  :telefonica_region       => ENV['OS_REGION_NAME'],
  :connection_options     => {:ssl_verify_peer => ENV['SSL_VERIFY'] != 'false'}
}

network_service = Fog::Network::TeleFonica.new(auth_options)
share_service   = Fog::SharedFileSystem::TeleFonica.new(auth_options)
