...
describe Fog::Image::TeleFonica do
  ...
  before :all do
    telefonica_vcr = TeleFonicaVCR.new(
      ...
      :service_class => Fog::Image::TeleFonica # Fog to choose latest available version
    )
  ...
  it "Image saving when image has been created with ID" do
    VCR.use_cassette('image_v2_save_id') do
      ...
      begin
        # Create an image with a specified ID
        foobar_image = Fog::Image::TeleFonica::V2::Image.new(
          :name               => 'original_name',
          :id                 => identifier,
          :service            => @service,
  ...
  it "Deactivates and activates an image" do
    VCR.use_cassette('image_v2_activation') do
      ...
      image_path = "spec/fixtures/telefonica/image_v2/minimal.ova"
