...
describe Fog::Identity::TeleFonica::V3 do
  before :all do
    @telefonica_vcr = TeleFonicaVCR.new(
      :vcr_directory => 'spec/fixtures/telefonica/identity_v3',
      :service_class => Fog::Identity::TeleFonica::V3
    )
    @service = @telefonica_vcr.service
    @os_auth_url = @telefonica_vcr.os_auth_url
  end

  it 'authenticates with password, userid and domain_id' do
    VCR.use_cassette('authv3_a') do
      Fog::Identity::TeleFonica::V3.new(
        :telefonica_domain_id => @telefonica_vcr.domain_id,
        :telefonica_api_key   => @telefonica_vcr.password,
        :telefonica_userid    => @telefonica_vcr.user_id,
        :telefonica_region    => @telefonica_vcr.region,
  ...
  it 'authenticates with password, username and domain_id' do
    VCR.use_cassette('authv3_b') do
      Fog::Identity::TeleFonica::V3.new(
        :telefonica_domain_id => @telefonica_vcr.domain_id,
        :telefonica_api_key   => @telefonica_vcr.password,
        :telefonica_username  => @telefonica_vcr.username,
        :telefonica_region    => @telefonica_vcr.region,
        :telefonica_auth_url  => @os_auth_url
      )
    end
  ...
  it 'authenticates with password, username and domain_name' do
    VCR.use_cassette('authv3_c') do
      Fog::Identity::TeleFonica::V3.new(
        :telefonica_user_domain => @telefonica_vcr.domain_name,
        :telefonica_api_key     => @telefonica_vcr.password,
        :telefonica_username    => @telefonica_vcr.username,
        :telefonica_region      => @telefonica_vcr.region,
        :telefonica_auth_url    => @os_auth_url
  ...
  it 'authenticates in another region' do
    ...
    endpoints_in_region = @endpoints_all.select { |endpoint| endpoint.region == @telefonica_vcr.region_other }

    unless endpoints_in_region.empty?
      VCR.use_cassette('idv3_other_region') do
        @fog = Fog::Identity::TeleFonica::V3.new(
          :telefonica_region   => @telefonica_vcr.region_other,
          :telefonica_auth_url => @os_auth_url,
          :telefonica_userid   => @telefonica_vcr.user_id,
          :telefonica_api_key  => @telefonica_vcr.password
        )
  ...
  it 'get an unscoped token, then reauthenticate with it' do
    VCR.use_cassette('authv3_unscoped_reauth') do
      id_v3 = Fog::Identity::TeleFonica::V3.new(
        :telefonica_api_key  => @telefonica_vcr.password,
        :telefonica_userid   => @telefonica_vcr.user_id,
        :telefonica_region   => @telefonica_vcr.region,
        :telefonica_auth_url => @os_auth_url
      ...
      auth_params = {
        :provider             => "telefonica",
        :telefonica_auth_token => id_v3.credentials[:telefonica_auth_token],
        :telefonica_auth_url   => @os_auth_url,
        :telefonica_region     => @telefonica_vcr.region
      }
      @fog2 = Fog::Identity::TeleFonica::V3.new(auth_params)

      @fog2.wont_equal nil
      token = @fog2.credentials[:telefonica_auth_token]
  ...
  it 'authenticates with project scope' do
    VCR.use_cassette('authv3_project') do
      Fog::Identity::TeleFonica::V3.new(
        :telefonica_project_name => @telefonica_vcr.project_name,
        :telefonica_domain_name  => @telefonica_vcr.domain_name,
        :telefonica_api_key      => @telefonica_vcr.password,
        :telefonica_username     => @telefonica_vcr.username,
        :telefonica_region       => @telefonica_vcr.region,
        :telefonica_auth_url     => @os_auth_url
  ...
  it 'get an unscoped token, then use it to get a scoped token' do
    VCR.use_cassette('authv3_unscoped') do
      ...
      auth = Fog::Identity::TeleFonica::V3.new(
        :telefonica_project_name => @telefonica_vcr.project_name,
        :telefonica_domain_name  => @telefonica_vcr.domain_name,
        :telefonica_auth_token   => id_v3.credentials[:telefonica_auth_token],
        :telefonica_region       => @telefonica_vcr.region,
      ...
      token = auth.credentials[:telefonica_auth_token]
      ...
      proc { id_v3.tokens.check('random-token') }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end

  it "find specific user, lists users" do
    VCR.use_cassette('idv3_users') do
      proc { @service.users.find_by_id 'u-random-blah' }.must_raise Fog::Identity::TeleFonica::NotFound

      admin_user = @service.users.find_by_name @telefonica_vcr.username
  ...
  it 'CRUD users' do
    VCR.use_cassette('idv3_user_crud') do
      ...
      proc { @service.users.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound
  ...
  it "CRUD & manipulate groups" do
    VCR.use_cassette('idv3_group_crud_mutation') do
      ...
      # Delete the group
      baz_group.destroy
      proc { @service.groups.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound
      @service.groups.all.select { |group| %w[foobar baz].include? group.name }.length.must_equal 0
    end
  end
  ...
  it "gets a token, checks it and then revokes it" do
    VCR.use_cassette('idv3_token') do
      auth = {
        :auth => {
          :identity => {
            ...
            :password => {
              :user => {
                :id       => @telefonica_vcr.user_id,
                :password => @telefonica_vcr.password
              }
            }
          },
          :scope    => {
            :project => {
              :domain => {
                :name => @telefonica_vcr.domain_name
              },
              :name   => @telefonica_vcr.project_name
            }
          }
        }
      }
      ...
      proc { @service.tokens.check(token.value) }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
    ...
    VCR.use_cassette('authv3_token') do
      ...
      begin
        foobar_user = @service.users.create(
          ...
          :domain_id => @telefonica_vcr.domain_id,
          :password  => 's3cret!'
        )
        ...
        nonadmin_v3 = Fog::Identity::TeleFonica::V3.new(
          :telefonica_domain_id => foobar_user.domain_id,
          :telefonica_api_key   => 's3cret!',
          :telefonica_username  => 'foobar_385',
          :telefonica_region    => @telefonica_vcr.region,
          :telefonica_auth_url  => auth_url
        )

        # Test - check the token validity by using it to create a new Fog::Identity::TeleFonica::V3 instance
        token_check = Fog::Identity::TeleFonica::V3.new(
          :telefonica_auth_token => nonadmin_v3.auth_token,
          :telefonica_region     => @telefonica_vcr.region,
          :telefonica_auth_url   => auth_url
        )
        ...
        proc do
          Fog::Identity::TeleFonica::V3.new(
            :telefonica_auth_token => 'blahblahblah',
            :telefonica_region     => @telefonica_vcr.region,
            :telefonica_auth_url   => auth_url
          )
  ...
  it "lists domains" do
    VCR.use_cassette('idv3_domain') do
      ...
      default_domain = @service.domains.find_by_id @telefonica_vcr.domain_id
      default_domain.wont_equal nil

      proc { @service.domains.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD domains" do
    VCR.use_cassette('idv3_domain_crud') do
      begin
        ...
        # Check that the deletion worked
        proc { @service.domains.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound if foobar_id
  ...
  it "lists roles" do
    VCR.use_cassette('idv3_role') do
      ...
      proc { @service.roles.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD roles" do
    VCR.use_cassette('idv3_role_crud') do
      begin
        ...
        baz_role = nil
        # Check that the deletion worked
        proc { @service.roles.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound if foobar_id
  ...
  it "lists projects" do
    VCR.use_cassette('idv3_project') do
      ...
      proc { @service.projects.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD projects" do
    VCR.use_cassette('idv3_project_crud') do
      ...
      begin
        ...
        # Check that the deletion worked
        proc { @service.projects.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound
  ...
  it "CRUD & list hierarchical projects" do
    VCR.use_cassette('idv3_project_hier_crud_list') do
      begin
        ...
        # Check that the deletion worked
        proc { @service.projects.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound if foobar_id
  ...
  it "lists services" do
    VCR.use_cassette('idv3_service') do
      ...
      proc { @service.services.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD services" do
    VCR.use_cassette('idv3_services_crud') do
      ...
      begin
        ...
        # Check that the deletion worked
        proc { @service.services.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound if foobar_id
  ...
  it "lists endpoints" do
    VCR.use_cassette('idv3_endpoint') do
      ...
      proc { @service.endpoints.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD endpoints" do
    VCR.use_cassette('idv3_endpoints_crud') do
      ...
      begin
        ...
        # Check that the deletion worked
        proc { @service.endpoints.find_by_id foobar_id }.must_raise Fog::Identity::TeleFonica::NotFound
  ...
  it "lists OS credentials" do
    VCR.use_cassette('idv3_credential') do
      ...
      proc { @service.os_credentials.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD OS credentials" do
    VCR.use_cassette('idv3_credential_crud') do
      begin
        ...
        # TeleFonica Keystone requires the blob to be a JSON string - i.e. not JSON, but a string containing JSON :-/
        ...
        # Check that the deletion worked
        if credential_id
          proc { @service.os_credentials.find_by_id credential_id }.must_raise Fog::Identity::TeleFonica::NotFound
        end
  ...
  it "lists policies" do
    VCR.use_cassette('idv3_policy') do
      ...
      proc { @service.policies.find_by_id 'atlantis' }.must_raise Fog::Identity::TeleFonica::NotFound
    end
  end
  ...
  it "CRUD policies" do
    VCR.use_cassette('idv3_policy_crud') do
      ...
      # Check that the deletion worked
      proc { @service.policies.find_by_id policy_id }.must_raise Fog::Identity::TeleFonica::NotFound
      @service.policies.all.select { |policy| policy.type == 'application/json' }.length.must_equal 0
    end
  end
end
