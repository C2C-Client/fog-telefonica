...
describe Fog::Image::TeleFonica do
  it "Upload/download image data using chunked IO" do
    ...
    if ENV['OS_AUTH_URL']
      ...
      @service ||= Fog::Image::TeleFonica.new(
        :telefonica_auth_url     => "#{@os_auth_url}/auth/tokens",
        :telefonica_project_name => ENV.fetch('OS_PROJECT_NAME'),
        :telefonica_username     => ENV.fetch('OS_USERNAME'),
        :telefonica_api_key      => ENV.fetch('OS_PASSWORD'),
        :telefonica_region       => ENV['OS_REGION_NAME'] || 'RegionOne',
        :telefonica_domain_name  => ENV['OS_USER_DOMAIN_NAME'] || 'Default'
      )

      spec_data_folder = 'spec/fixtures/telefonica/image_v2'
      ...
      begin
        ...
        if foobar_id
          ...
          end.must_raise Fog::Image::TeleFonica::NotFound
        end
