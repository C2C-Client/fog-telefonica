...
[
  Fog::Volume::TeleFonica,
  Fog::Volume::TeleFonica::V1,
  Fog::Volume::TeleFonica::V2
].delete_if { |the_class| ENV['TEST_CLASS'] && ENV['TEST_CLASS'] != the_class.name }.each do |service_class|
  describe service_class do
    before :all do
      vcr_directory = 'spec/fixtures/telefonica/volume' if service_class == Fog::Volume::TeleFonica
      vcr_directory = 'spec/fixtures/telefonica/volume_v1' if service_class == Fog::Volume::TeleFonica::V1
      vcr_directory = 'spec/fixtures/telefonica/volume_v2' if service_class == Fog::Volume::TeleFonica::V2
    ...
    def v2?
      @service.kind_of? Fog::Volume::TeleFonica::V2::Real
    end
    ...
    it 'CRUD volumes' do
      VCR.use_cassette('volume_crud') do
        begin
          ...
          volume.must_be_kind_of Fog::Volume::TeleFonica::Volume
    ...
    it 'reads volume types' do
      VCR.use_cassette('volume_type_read') do
        ...
        type.must_be_kind_of Fog::Volume::TeleFonica::VolumeType
    ...
    it 'can extend volumes' do
      VCR.use_cassette('volume_extend') do
        begin
          ...
          # shrinking is not allowed in TeleFonica
          ...
          proc { @service.extend_volume(volume.id, volume_size_small) }.must_raise Fog::Volume::TeleFonica::NotFound
        end
      end
    end
    ...
    it 'can create and accept volume transfers' do
      VCR.use_cassette('volume_transfer_and_accept') do
        begin
          ...
          transfer.must_be_kind_of Fog::Volume::TeleFonica::Transfer
          ...
          other_service = service_class.new(
            ...
            :telefonica_region       => ENV['OS_REGION_NAME'] || 'RegionOne',
            :telefonica_api_key      => ENV['OS_PASSWORD_OTHER'] || 'password',
            :telefonica_username     => ENV['OS_USERNAME_OTHER'] || 'demo',
            :telefonica_domain_name  => ENV['OS_USER_DOMAIN_NAME'] || 'Default',
            :telefonica_project_name => ENV['OS_PROJECT_NAME_OTHER'] || 'demo'
          )
          ...
          # proc { other_service.volumes.get(volume.id) }.must_raise Fog::Compute::TeleFonica::NotFound
          ...
          proc { other_service.transfers.accept(bogus_uuid, auth_key) }.must_raise Fog::Volume::TeleFonica::NotFound
          ...
          # proc { @service.volumes.get(volume.id) }.must_raise Fog::Compute::TeleFonica::NotFound
          # @service.volumes.all(@name_param => volume_name).length.must_equal 0
      ...
      if v2?
        VCR.use_cassette('volume_transfer_and_delete') do
          begin
            ...
            proc { other_service.transfers.accept(transfer_id, auth_key) }.must_raise Fog::Volume::TeleFonica::NotFound
          ensure
            # cleanup volume
