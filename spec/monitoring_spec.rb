...
describe Fog::Monitoring::TeleFonica do
  spec_data_folder = 'spec/fixtures/telefonica/monitoring'

  before :all do
    telefonica_vcr = TeleFonicaVCR.new(
      :vcr_directory => spec_data_folder,
      :service_class => Fog::Monitoring::TeleFonica
    )
  ...
  it 'metric crud tests' do
    VCR.use_cassette('metric_crud') do
      ...
      metric_1 = Fog::Monitoring::TeleFonica::Metric.new(:name       => 'sample_metric_1',
                                                        :timestamp  => @timestamp,
                                                        :dimensions => {"key1" => "value1"},
                                                        :value      => 42,
                                                        :value_meta => {"meta_key1" => "meta_value1"})

      metric_2 = Fog::Monitoring::TeleFonica::Metric.new(:name       => 'sample_metric_2',
                                                        :timestamp  => @timestamp,
                                                        :dimensions => {"key1" => "value1"},
                                                        :value      => 42,
                                                        :value_meta => {"meta_key1" => "meta_value1"})
  ...
  it 'notification methods crud test' do
    VCR.use_cassette('notification-methods_crud') do
      begin
        ...
        end.must_raise Fog::Monitoring::TeleFonica::NotFound
      ensure
  ...
  # FIXME: According to the API (https://github.com/telefonica/monasca-api/blob/master/docs/monasca-api-spec.md)
