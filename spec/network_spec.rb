...
describe Fog::Network::TeleFonica do
  before :all do
    telefonica_vcr = TeleFonicaVCR.new(
      :vcr_directory  => 'spec/fixtures/telefonica/network',
      :service_class  => Fog::Network::TeleFonica,
      :project_scoped => true
    )
    @service          = telefonica_vcr.service
    @current_project  = telefonica_vcr.project_name

    telefonica_vcr = TeleFonicaVCR.new(
      :vcr_directory => 'spec/fixtures/telefonica/network',
      :service_class => Fog::Identity::TeleFonica::V3
    )
    @identity_service = telefonica_vcr.service
  end
  ...
  it 'fails at token expiration on auth with token but not with username+password' do
    VCR.use_cassette('token_expiration') do
      @auth_token = @identity_service.credentials[:telefonica_auth_token]
      telefonica_vcr = TeleFonicaVCR.new(
        :vcr_directory  => 'spec/fixtures/telefonica/network',
        :service_class  => Fog::Network::TeleFonica,
        :project_scoped => true,
      ...
      @service_with_token = telefonica_vcr.service

      [@service_with_token, @service].each_with_index do |service, index|
        @network_token = service.credentials[:telefonica_auth_token]
