...
describe "Fog::Identity[:telefonica] | users" do
  before do
  ...
  describe "fails" do
    it "#find_by_id" do
      unless Fog.mocking?
        proc do
          Fog::Identity[:telefonica].users.find_by_id('fake')
    ...
    it "#find_by_name" do
      unless Fog.mocking?
        proc do
          Fog::Identity[:telefonica].users.find_by_name('fake')
        end.must_raise(Fog::Identity::TeleFonica::NotFound)
      end
    end
    ...
    it "#destroy" do
      unless Fog.mocking?
        proc do
          Fog::Identity[:telefonica].users.destroy('fake')
        end.must_raise(Fog::Identity::TeleFonica::NotFound)
      end
    end
  end
