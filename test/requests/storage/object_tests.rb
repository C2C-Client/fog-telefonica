...
describe "Fog::Storage[:telefonica] | object requests" do
  before do
    unless Fog.mocking?
      @directory = Fog::Storage[:telefonica].directories.create(:key => 'fogobjecttests')
    end

    module TeleFonicaStorageHelpers
  ...
  describe "success" do
    it "#put_object('fogobjecttests', 'fog_object')" do
      resp = Fog::Storage[:telefonica].put_object('fogobjecttests', 'fog_object', lorem_file)
    ...
    describe "with_object" do
      before do
        file = lorem_file
        resp = Fog::Storage[:telefonica].put_object('fogobjecttests', 'fog_object', file)
        file.close
        resp.headers['ETag'].must_equal '80d7930fe13ff4e45156b6581656a247'
      end
      ...
      it "#get_object('fogobjectests', 'fog_object')" do
        unless Fog.mocking?
          resp = Fog::Storage[:telefonica].get_object('fogobjecttests', 'fog_object')
      ...
      it "#get_object('fogobjecttests', 'fog_object', &block)" do
        unless Fog.mocking?
          data = ''
          Fog::Storage[:telefonica].get_object('fogobjecttests', 'fog_object') do |chunk, _remaining_bytes, _total_bytes|
            data << chunk
          end
      ...
      it "#public_url('fogobjectests', 'fog_object')" do
        unless Fog.mocking?
          url = Fog::Storage[:telefonica].directories.first.files.first.public_url
      ...
      it "#public_url('fogobjectests')" do
        unless Fog.mocking?
          url = Fog::Storage[:telefonica].directories.first.public_url
      ...
      it "#head_object('fogobjectests', 'fog_object')" do
        unless Fog.mocking?
          resp = Fog::Storage[:telefonica].head_object('fogobjecttests', 'fog_object')
          resp.headers['ETag'].must_equal '80d7930fe13ff4e45156b6581656a247'
        end
      end
      ...
      it "#post_object('fogobjecttests', 'fog_object')" do
        unless Fog.mocking?
          Fog::Storage[:telefonica].post_object(
            'fogobjecttests',
            'fog_object',
      ...
      it "#delete_object('fogobjecttests', 'fog_object')" do
        unless Fog.mocking?
          resp = Fog::Storage[:telefonica].delete_object('fogobjecttests', 'fog_object')
      ...
      it "#get_object_http_url('directory.identity', 'fog_object', expiration timestamp)" do
        unless Fog.mocking?
          ...
          url_s = Fog::Storage[:telefonica].get_object_http_url(@directory.identity, 'fog_object', ts)
      ...
      it "#get_object_https_url('directory.identity', 'fog_object', expiration timestamp)" do
        unless Fog.mocking?
          ts = Time.at(1_395_343_213)
          url_s = Fog::Storage[:telefonica].get_object_https_url(@directory.identity, 'fog_object', ts)
      ...
      it "#get_object_https_url_numeric('directory.identity', 'fog_object', expiration_timestamp)" do
        unless Fog.mocking?
          ...
          fog = Fog::Storage.new(:provider => :telefonica, :telefonica_temp_url_key => '12345')
    ...
    describe "put_object with block" do
      it "#put_object('fogobjecttests', 'fog_object', &block)" do
        begin
          ...
          resp = Fog::Storage[:telefonica].put_object('fogobjecttests', 'fog_block_object', nil) do
      ...
      describe "with_object" do
        before do
          file = lorem_file
          Fog::Storage[:telefonica].put_object('fogobjecttests', 'fog_block_object', nil) do
        ...
        it "#get_object" do
          unless Fog.mocking?
            resp = Fog::Storage[:telefonica].get_object('fogobjecttests', 'fog_block_object')
            resp.body.must_equal lorem_file.read
          end
        end
        ...
        it "#delete_object" do
          unless Fog.mocking?
            resp = Fog::Storage[:telefonica].delete_object('fogobjecttests', 'fog_block_object')
            resp.status.must_equal 204
          end
        end
      end
    ...
    describe "deletes multiple objects" do
      before do
        unless Fog.mocking?
          Fog::Storage[:telefonica].put_object('fogobjecttests', 'fog_object', lorem_file)
          Fog::Storage[:telefonica].put_object('fogobjecttests', 'fog_object2', lorem_file)
          Fog::Storage[:telefonica].directories.create(:key => 'fogobjecttests2')
          Fog::Storage[:telefonica].put_object('fogobjecttests2', 'fog_object', lorem_file)
        end
      ...
      after do
        unless Fog.mocking?
          dir2 = Fog::Storage[:telefonica].directories.get('fogobjecttests2')
          unless dir2.nil?
      ...
      it "#delete_multiple_objects" do
        unless Fog.mocking?
          resp = Fog::Storage[:telefonica].delete_multiple_objects(
  ...
  describe "failure" do
    it "#get_object('fogobjecttests', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].get_object('fogobjecttests', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
    ...
    it "#get_object('fognoncontainer', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].get_object('fognoncontainer', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
    ...
    it "#head_object('fogobjecttests', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].head_object('fogobjecttests', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
    ...
    it "#head_object('fognoncontainer', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].head_object('fognoncontainer', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
    ...
    it "#post_object('fognoncontainer', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].post_object('fognoncontainer', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
    ...
    it "#delete_object('fogobjecttests', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].delete_object('fogobjecttests', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
    ...
    it "#delete_object('fognoncontainer', 'fog_non_object')" do
      unless Fog.mocking?
        proc do
          Fog::Storage[:telefonica].delete_object('fognoncontainer', 'fog_non_object')
        end.must_raise(Fog::Storage::TeleFonica::NotFound)
      end
    end
