...
describe "Fog::Image[:telefonica] | image requests" do
  before(:all) do
    telefonica = Fog::Identity::TeleFonica.new(:telefonica_identity_api_version => 'v2.0')

    @image_attributes = {
      :name             => 'new image',
      :owner            => telefonica.current_tenant['id'],
      :is_public        => true,
      :copy_from        => 'http://website.com/image.iso',
      :disk_format      => 'iso',
      :properties       => {
        :user_id  => telefonica.current_user['id'],
        :owner_id => telefonica.current_tenant['id']
      },
    ...
    @instance = Fog::Image[:telefonica].create_image(image_attributes).body
  end
  ...
  describe "success" do
    it "#list_public_images" do
      Fog::Image[:telefonica].list_public_images.body.must_match_schema('images' => [@image_format])
    end

    it "#list_public_images_detailed" do
      Fog::Image[:telefonica].list_public_images_detailed.body.
    ...
    it "#get_image" do
      Fog::Image[:telefonica].get_image(@instance['image']['id']).headers.
        must_match_schema(@image_meta_format)
    end

    it "#update_image" do
      Fog::Image[:telefonica].update_image(
    ...
    it "#add_member_to_image" do
      [200, 204].must_include(
        Fog::Image[:telefonica].add_member_to_image(
    ...
    it "#get_image_members" do
      [200, 204].must_include(Fog::Image[:telefonica].get_image_members(@instance['image']['id']).status)
    end

    it "#get_shared_images" do
      [200, 204].must_include(Fog::Image[:telefonica].get_shared_images(@instance['image']['owner']).status)
    end

    it "#remove_member_from_image" do
      [200, 204].must_include(
        Fog::Image[:telefonica].remove_member_from_image(
          @instance['image']['id'], @instance['image']['owner']
        ).status
      )
    end
    ...
    it "#delete_image" do
      Fog::Image[:telefonica].delete_image(@instance['image']['id']).status.must_equal 200
    end
  end
end
