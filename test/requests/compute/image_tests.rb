...
describe "Fog::Compute[:telefonica] | image requests" do
  before do
    @image_format = {
    ...
    @compute = Fog::Compute[:telefonica]
  end

  describe "success" do
    before do
      @image_id = Fog::Compute[:telefonica].images[0].id
      unless Fog.mocking?
    ...
    it "#list_images" do
      @compute.list_images.body.
        must_match_schema('images' => [TeleFonica::Compute::Formats::SUMMARY])
    end
