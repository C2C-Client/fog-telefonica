...
describe "Fog::Compute[:telefonica] | server requests" do
  def self.compute
    class_variable_get(:@@compute)
  end

  class_variable_set(:@@compute, Fog::Compute[:telefonica])
  ...
  describe "success" do
    ...
    describe "single server from image" do
      before do
        ...
        #  Fog::Compute[:telefonica].servers.get(@server_id).wait_for { ready? }
      end
    ...
    it "#list_servers" do
      compute.list_servers.body.
        must_match_schema({'servers' => [TeleFonica::Compute::Formats::SUMMARY]},
                          nil,
                          :allow_extra_keys     => true,
                          :allow_optional_rules => true)
    end
  ...
  describe "failure" do
    it "#delete_server(0)" do
      ...
      end.must_raise Fog::Compute::TeleFonica::NotFound
    end
