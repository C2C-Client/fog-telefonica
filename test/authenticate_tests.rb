...
describe "TeleFonica authentication" do
  before do
  ...
  it "with v2" do
    ...
    assert(expected) do
      Fog::TeleFonica.authenticate_v2(
        :telefonica_auth_uri     => URI('http://example'),
        :telefonica_tenant       => 'admin',
        :telefonica_service_type => %w[compute])
    end
  end

  it "validates token" do
    creds = {
      :telefonica_auth_url => 'http://telefonica:35357',
      :telefonica_identity_api_version => 'v2.0'
    }
    identity = Fog::Identity::TeleFonica.new(creds)
  ...
  it "v2 missing service" do
    ...
    service = Object.new
    service.extend(Fog::TeleFonica::Core)
    service.send(
      :setup,
      :telefonica_auth_url     => 'http://example',
      :telefonica_tenant       => 'admin',
      :telefonica_service_type => %w[network],
      :telefonica_api_key      => 'secret',
      :telefonica_username     => 'user')
    proc do
      service.send(:authenticate)
    end.must_raise Fog::TeleFonica::Auth::Catalog::ServiceTypeError
  end
  ...
  it "v2 missing storage service" do
    ...
    service.send(
      ...
      :telefonica_api_key      => 'secret',
      :telefonica_username     => 'user',
      :telefonica_service_type => 'object-store')
  ...
  it "v2 auth with two compute services" do
    ...
    service.send(
      ...
      :telefonica_api_key      => 'secret',
      :telefonica_username     => 'user',
      :telefonica_service_type => 'compute')

    proc do
      service.send(:authenticate)
    end.must_raise Fog::TeleFonica::Auth::Catalog::EndpointError, 'Multiple endpoints found'
  end
