...
describe Fog::TeleFonica::Auth::Name do
  describe 'creates' do
    it 'when id and name are provided' do
      name = Fog::TeleFonica::Auth::Name.new('default', 'Default')
      name.id.must_equal 'default'
      name.name.must_equal 'Default'
    end

    it 'when id is null' do
      name = Fog::TeleFonica::Auth::Name.new(nil, 'Default')
      name.id.must_be_nil
      name.name.must_equal 'Default'
    end

    it 'when name is null' do
      name = Fog::TeleFonica::Auth::Name.new('default', nil)
      name.id.must_equal 'default'
      name.name.must_be_nil
    end

    it 'when both id and name is null' do
      name = Fog::TeleFonica::Auth::Name.new(nil, nil)
      name.name.must_be_nil
    end
  end
...
describe Fog::TeleFonica::Auth::User do
  describe '#password' do
    it 'set/get password' do
      user = Fog::TeleFonica::Auth::User.new('user_id', 'User')
  ...
  describe '#identity' do
    describe 'succesful' do
      ...
      it 'with user name and user domain name' do
        user = Fog::TeleFonica::Auth::User.new(nil, 'User')
        user.password = 'secret'
        user.domain = Fog::TeleFonica::Auth::Name.new('default', nil)
      ...
      it 'with user name and domain name' do
        user = Fog::TeleFonica::Auth::User.new(nil, 'User')
        user.password = 'secret'
        user.domain = Fog::TeleFonica::Auth::Name.new(nil, 'Default')
    ...
    describe 'raises an error' do
      it 'raises an error when password is missing' do
        proc do
          user = Fog::TeleFonica::Auth::User.new('user_id', 'User')
          user.identity
        end.must_raise Fog::TeleFonica::Auth::CredentialsError
      end
...
describe Fog::TeleFonica::Auth::ProjectScope do
  describe '#identity' do
    it "when id is provided it doesn't require domain" do
      project = Fog::TeleFonica::Auth::ProjectScope.new('project_id', 'Project')
      project.identity.must_equal(:project => {:id => 'project_id'})
    end

    it 'when id is nul and name is provided it uses domain id' do
      project = Fog::TeleFonica::Auth::ProjectScope.new(nil, 'Project')
      project.domain = Fog::TeleFonica::Auth::Name.new('default', nil)
    ...
    it 'when id is nul and name is provided it uses domain name' do
      project = Fog::TeleFonica::Auth::ProjectScope.new(nil, 'Project')
      project.domain = Fog::TeleFonica::Auth::Name.new(nil, 'Default')
